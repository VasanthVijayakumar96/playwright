...
Then(
  ...
  async function (this: CustomWorld) {
    await expect(
      this.page.locator('[data-testid="register-page"]')
    ).toBeVisible();
    await expect(
...
Then("the navbar profile should be visible", async function (this: CustomWorld) {
  await expect(this.page.locator(".navbar-profile")).toBeVisible({
    timeout: 10000
  });
});

Then("the logged out navbar should be visible", async function (this: CustomWorld) {
...
When("I logout via navbar", async function (this: CustomWorld) {
