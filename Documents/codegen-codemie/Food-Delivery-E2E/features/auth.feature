...
Feature: Authentication flows
  ...
  # Navigate to /cart, then click Sign-in so returnTo="/cart"; verify redirect back to /cart.
  @smoke @TC-AUTH-003
  ...
  # Form relies on browser-native required + type="email" validation.
  # No custom error element exists in the DOM; assertion is URL unchanged.
  ...
  # Requires seeded test user (E2E_USER_EMAIL) for deterministic wrong-password
  # result. Server error is surfaced via react-toastify (.Toastify__toast).
  ...
  # Browser type="email" format check fires at submit; "not-an-email" is rejected.
