...
Then(
  ...
  async function (this: FoodDeliveryWorld, fragment: string) {
    ...
    expect(url, `Expected URL to contain "${fragment}" but got "${url}"`).toContain(
      fragment
    );
  }
);
...
Then(
  ...
  async function (this: FoodDeliveryWorld, selector: string) {
    ...
    const inViewport = await loc.evaluate((el: Element) => {
      ...
      return (
        rect.top >= 0 && rect.top <= vh && rect.left >= 0 && rect.left <= vw
      );
    });
...
Then(
  ...
  async function (this: FoodDeliveryWorld) {
    ...
    expect(realErrors, `Fatal console errors detected:\n${realErrors.join("\n")}`).toEqual(
      []
    );
  }
);
